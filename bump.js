...
appJson.expo.version = '1.3.3';
appJson.expo.android.versionCode = 105;
fs.writeFileSync('app.json', JSON.stringify(appJson, null, 2));

let pkgJson = JSON.parse(fs.readFileSync('package.json'));
pkgJson.version = '1.3.3';
fs.writeFileSync('package.json', JSON.stringify(pkgJson, null, 2));

let updateTs = fs.readFileSync('src/update.ts', 'utf8');
updateTs = updateTs.replace(/export const APP_VERSION = ".*";/, 'export const APP_VERSION = "1.3.3";');
